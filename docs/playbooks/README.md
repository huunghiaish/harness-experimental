...
| File | One-line problem |
|------|------------------|
| [solo-dev-client-delivery.md](solo-dev-client-delivery.md) | Meta-playbook for solo-dev (or small-team) paid client delivery. Maps a 13-stage commercial flow (lead → intake brief → discovery → SOW → spec intake → visual & behavioral modeling → story slicing → build → review → QA → UAT → release → handover + maintenance) onto existing harness pieces. Pointers only; composes, does not duplicate. |
| [visual-and-behavioral-modeling.md](visual-and-behavioral-modeling.md) | Stage 6 of solo-dev-client-delivery. Three sub-steps: design system check, interactive prototype (Claude Design / Stitch / v0.dev fallback ladder), business diagrams (sitemap, user flow, business workflow, ERD draft, role-permission matrix, status flow). Output under `docs/visuals/`. Triple-use: client review + AI code handoff + UAT pass criterion. |
...
| [project-status-snapshot.md](project-status-snapshot.md) | Read-only "where are we now" snapshot. Agent reads stories + TEST_MATRIX + decisions + playbook lifecycles; emits 4-section markdown report + one-line summary. No script (per decision 0005 § 6). |
